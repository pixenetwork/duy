...
This repository allows multiple AI assistants, but each branch/PR has one
primary writer.
...
- **CodeRabbit:** external automated PR review after checks are green and the
  diff is stable.
...
4. Run available local validation (tests/build/typecheck/lint/static scripts)
   and record exact outputs.
5. Open or keep a draft PR while work is in progress.
6. **CI-before-review escalation:** if CI fails, fix or escalate CI blockers
   before requesting/retriggering external review.
7. **Review-budget backoff:** when reviews become noisy or repetitive, pause
   new review requests, stabilize the diff, then request one fresh review.
...
- For FiveM work, keep authorization and persistent-state decisions
  server-authoritative and validate untrusted client input.
