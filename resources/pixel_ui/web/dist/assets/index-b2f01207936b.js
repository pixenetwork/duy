/*! Pixel UI v0.2.1 | React 18.2.0 runtime | Enhanced-only */
...
;(function bootstrapPixelReactRuntime(globalObject) {
  ...
  globalObject.React = pixelRequire(96540);
  globalObject.ReactDOM = pixelRequire(40961);
...
var PixelUI;
(function (PixelUI) {
    PixelUI.isUnknownRecord = (value) => typeof value === 'object' && value !== null && !Array.isArray(value);
    PixelUI.isString = (value) => typeof value === 'string';
    PixelUI.isNonEmptyString = (value) => typeof value === 'string' && value.trim().length > 0;
    PixelUI.isBoolean = (value) => typeof value === 'boolean';
    PixelUI.isFiniteNumber = (value) => typeof value === 'number' && Number.isFinite(value);
    function isOptional(guard) {
        return (value) => value === undefined || guard(value);
    }
    PixelUI.isOptional = isOptional;
    function isArrayOf(guard) {
        return (value) => Array.isArray(value) && value.every(guard);
    }
    PixelUI.isArrayOf = isArrayOf;
    function validate(value, guard, label = 'payload') {
        return guard(value)
            ? { ok: true, value }
            : { ok: false, issues: [`Invalid ${label}`] };
    }
    PixelUI.validate = validate;
    PixelUI.isRequestId = (value) => typeof value === 'string' && /^[A-Za-z0-9:_-]{8,96}$/.test(value);
    PixelUI.isBridgeError = (value) => {
        if (!PixelUI.isUnknownRecord(value))
            return false;
        const codes = [
            'ABORTED',
            'BAD_REQUEST',
        ...
        return typeof value.code === 'string'
            ...
            && PixelUI.isNonEmptyString(value.message);
    };
    PixelUI.isNuiEventEnvelope = (value) => PixelUI.isUnknownRecord(value)
        && value.version === 1
        && PixelUI.isNonEmptyString(value.event)
        && 'payload' in value;
    PixelUI.isNuiResponseEnvelope = (value) => {
        if (!PixelUI.isUnknownRecord(value) || value.version !== 1 || !PixelUI.isRequestId(value.requestId))
            return false;
        if (value.ok === true)
            return 'data' in value;
        return value.ok === false && PixelUI.isBridgeError(value.error);
    };
    PixelUI.isUiOwner = (value) => PixelUI.isUnknownRecord(value)
        && PixelUI.isNonEmptyString(value.id)
        && PixelUI.isBoolean(value.focus)
        && PixelUI.isBoolean(value.cursor)
        && (value.panel === undefined || PixelUI.isString(value.panel));
    PixelUI.isUiStateSnapshot = (value) => PixelUI.isUnknownRecord(value)
        && PixelUI.isBoolean(value.visible)
        && (value.activeOwner === null || PixelUI.isNonEmptyString(value.activeOwner))
        && PixelUI.isBoolean(value.focus)
        && PixelUI.isBoolean(value.cursor)
        && PixelUI.isFiniteNumber(value.modalDepth)
        && value.modalDepth >= 0
        && PixelUI.isBoolean(value.showcaseEnabled)
        && isArrayOf(PixelUI.isUiOwner)(value.owners)
        && PixelUI.isFiniteNumber(value.revision);
    PixelUI.isDiagnosticEvent = (value) => {
        if (!PixelUI.isUnknownRecord(value))
            return false;
        return ['debug', 'info', 'warn', 'error'].includes(String(value.level))
            && PixelUI.isNonEmptyString(value.resource)
            && PixelUI.isNonEmptyString(value.module)
            && PixelUI.isNonEmptyString(value.message)
            && (value.fields === undefined || PixelUI.isUnknownRecord(value.fields));
    };
    PixelUI.isReadyResponse = (value) => PixelUI.isUnknownRecord(value) && PixelUI.isNonEmptyString(value.version) && PixelUI.isUiStateSnapshot(value.state);
    PixelUI.isOkResponse = (value) => PixelUI.isUnknownRecord(value) && PixelUI.isBoolean(value.accepted) && PixelUI.isUiStateSnapshot(value.state);
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    class UiOwnershipState {
        constructor() {
            this.owners = new Map();
            this.order = [];
            this.revision = 0;
    ...
    PixelUI.UiOwnershipState = UiOwnershipState;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    function nextKeyboardIndex(current, count, key, orientation = 'both') {
        if (count <= 0)
            return -1;
        if (key === 'Home')
            return 0;
    ...
    PixelUI.nextKeyboardIndex = nextKeyboardIndex;
    function shouldCloseOnEscape(key, isTopMost, dismissible = true) {
        return key === 'Escape' && isTopMost && dismissible;
    }
    PixelUI.shouldCloseOnEscape = shouldCloseOnEscape;
    PixelUI.FOCUSABLE_SELECTOR = [
        'a[href]',
        'button:not([disabled])',
        'input:not([disabled])',
        'select:not([disabled])',
    ...
    function activateFocusTrap(container, onEscape, isTopMost) {
        const previouslyFocused = document.activeElement instanceof HTMLElement
            ? document.activeElement
            : null;
        const focusables = () => Array.from(container.querySelectorAll(PixelUI.FOCUSABLE_SELECTOR))
    ...
    PixelUI.activateFocusTrap = activateFocusTrap;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    ...
    PixelUI.BridgeFailure = BridgeFailure;
    const eventGuards = {
        'pixel.ui.state': PixelUI.isUiStateSnapshot,
        'pixel.ui.closeAll': (value) => PixelUI.isUnknownRecord(value) && PixelUI.isNonEmptyString(value.reason),
        'pixel.diagnostics': PixelUI.isDiagnosticEvent,
    };
    const callbackGuards = {
        'ui.ready': {
            request: (value) => PixelUI.isUnknownRecord(value) && Object.keys(value).length === 0,
            response: PixelUI.isReadyResponse,
        },
        'ui.close': {
            request: (value) => PixelUI.isUnknownRecord(value) && PixelUI.isNonEmptyString(value.owner),
            response: PixelUI.isOkResponse,
        },
        'ui.acquire': {
            request: (value) => PixelUI.isUnknownRecord(value)
                && PixelUI.isNonEmptyString(value.owner)
                && PixelUI.isBoolean(value.focus)
                && PixelUI.isBoolean(value.cursor)
                && (value.panel === undefined || PixelUI.isString(value.panel)),
            response: PixelUI.isOkResponse,
        ...
        'ui.closeAll': {
            request: (value) => PixelUI.isUnknownRecord(value) && (value.reason === undefined || PixelUI.isString(value.reason)),
            response: PixelUI.isOkResponse,
        },
        'ui.modalDepth': {
            request: (value) => PixelUI.isUnknownRecord(value) && PixelUI.isFiniteNumber(value.depth) && value.depth >= 0,
            response: PixelUI.isOkResponse,
        },
        'diagnostics.example': {
            request: (value) => PixelUI.isUnknownRecord(value) && PixelUI.isNonEmptyString(value.message) && value.message.length <= 160,
            response: (value) => PixelUI.isUnknownRecord(value) && PixelUI.isBoolean(value.accepted) && PixelUI.isString(value.echoed),
        },
    };
    class NuiEventBus {
        constructor(diagnostic = () => undefined) {
        ...
        receive(value) {
            if (!PixelUI.isNuiEventEnvelope(value)) {
            ...
            for (const subscription of [...listeners]) {
                subscription.listener(value.payload);
    ...
    PixelUI.NuiEventBus = NuiEventBus;
    class NuiClient {
        constructor(transport, diagnostic = () => undefined) {
        ...
        registerMock(name, handler) {
            this.mocks.set(name, handler);
            return () => this.mocks.delete(name);
        }
        request(callback, payload, options = {}) {
            if (this.closed) {
            ...
            const envelope = { version: 1, requestId, callback, payload };
            ...
            return new Promise((resolve, reject) => {
                ...
                this.pending.set(requestId, {
                    callback,
                    responseGuard: callbackGuards[callback].response,
                    resolve: resolve,
                    reject,
                    timer,
        ...
        receiveResponse(value) {
            if (!PixelUI.isNuiResponseEnvelope(value)) {
        ...
        close(reason = 'NUI bridge closed') {
            if (this.closed)
                return;
            this.closed = true;
        ...
        cancelPending(reason = 'NUI interface closed') {
        ...
        async defaultTransport(envelope, signal) {
            const mock = this.mocks.get(envelope.callback);
            if (mock && typeof window.GetParentResourceName !== 'function') {
                const data = await mock(envelope.payload);
    ...
    PixelUI.NuiClient = NuiClient;
    function createBrowserMockState() {
        return {
            visible: true,
    ...
    PixelUI.createBrowserMockState = createBrowserMockState;
    function installBrowserMocks(client) {
        if (typeof window.GetParentResourceName === 'function')
            return;
        const state = new PixelUI.UiOwnershipState();
    ...
    PixelUI.installBrowserMocks = installBrowserMocks;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    function Button({ tone = 'primary', size = 'medium', loading = false, fullWidth = false, children, className, disabled, ...props }) {
        return (React.createElement("button", { ...props, type: props.type ?? 'button', className: classNames('pixel-button', `pixel-tone--${tone}`, `pixel-size--${size}`, fullWidth && 'pixel-control--full', className), disabled: disabled || loading, "aria-busy": loading || undefined },
            loading && React.createElement("span", { className: "pixel-spinner pixel-spinner--inline", "aria-hidden": "true" }),
            React.createElement("span", null, children)));
    }
    PixelUI.Button = Button;
    function IconButton({ label, icon, className, ...props }) {
        return (React.createElement(Button, { ...props, className: classNames('pixel-icon-button', className), "aria-label": label },
            React.createElement("span", { "aria-hidden": "true" }, icon)));
    }
    PixelUI.IconButton = IconButton;
    function FieldFrame({ id, label, hint, error, children }) {
        const describedBy = [hint && `${id}-hint`, error && `${id}-error`].filter(Boolean).join(' ') || undefined;
        return (React.createElement("label", { className: "pixel-field", htmlFor: id },
            React.createElement("span", { className: "pixel-field__label" }, label),
            React.createElement('span', { className: 'pixel-field__control', 'aria-describedby': describedBy }, children),
            hint && React.createElement("span", { id: `${id}-hint`, className: "pixel-field__hint" }, hint),
            error && React.createElement("span", { id: `${id}-error`, className: "pixel-field__error", role: "alert" }, error)));
    }
    function Input({ label, hint, error, id: suppliedId, className, ...props }) {
        const generatedId = React.useId();
        const id = suppliedId ?? generatedId;
        const describedBy = [hint && `${id}-hint`, error && `${id}-error`].filter(Boolean).join(' ') || undefined;
        return (React.createElement(FieldFrame, { id: id, label: label, hint: hint, error: error },
            React.createElement("input", { ...props, id: id, className: classNames('pixel-input', className), "aria-invalid": Boolean(error) || undefined, "aria-describedby": describedBy })));
    }
    PixelUI.Input = Input;
    function Textarea({ label, hint, error, id: suppliedId, className, ...props }) {
        const generatedId = React.useId();
        const id = suppliedId ?? generatedId;
        const describedBy = [hint && `${id}-hint`, error && `${id}-error`].filter(Boolean).join(' ') || undefined;
        return (React.createElement(FieldFrame, { id: id, label: label, hint: hint, error: error },
            React.createElement("textarea", { ...props, id: id, className: classNames('pixel-input pixel-textarea', className), "aria-invalid": Boolean(error) || undefined, "aria-describedby": describedBy })));
    }
    PixelUI.Textarea = Textarea;
    function Select({ label, hint, error, options, id: suppliedId, className, ...props }) {
        const generatedId = React.useId();
        const id = suppliedId ?? generatedId;
        const describedBy = [hint && `${id}-hint`, error && `${id}-error`].filter(Boolean).join(' ') || undefined;
        return (React.createElement(FieldFrame, { id: id, label: label, hint: hint, error: error },
            React.createElement("select", { ...props, id: id, className: classNames('pixel-input pixel-select', className), "aria-invalid": Boolean(error) || undefined, "aria-describedby": describedBy }, options.map((option) => (React.createElement("option", { key: option.value, value: option.value, disabled: option.disabled }, option.label))))));
    }
    PixelUI.Select = Select;
    function Checkbox({ label, description, id: suppliedId, className, ...props }) {
        const generatedId = React.useId();
        const id = suppliedId ?? generatedId;
        return (React.createElement("label", { className: classNames('pixel-check', className), htmlFor: id },
            React.createElement("input", { ...props, id: id, type: "checkbox", className: "pixel-check__native" }),
            React.createElement("span", { className: "pixel-check__box", "aria-hidden": "true" }, "\u2713"),
            React.createElement("span", null,
                React.createElement("span", { className: "pixel-check__label" }, label),
                description && React.createElement("span", { className: "pixel-check__description" }, description))));
    }
    PixelUI.Checkbox = Checkbox;
    function Toggle({ label, description, id: suppliedId, className, ...props }) {
        const generatedId = React.useId();
        const id = suppliedId ?? generatedId;
        return (React.createElement("label", { className: classNames('pixel-toggle', className), htmlFor: id },
            React.createElement("span", null,
                React.createElement("span", { className: "pixel-check__label" }, label),
                description && React.createElement("span", { className: "pixel-check__description" }, description)),
            React.createElement("input", { ...props, id: id, type: "checkbox", role: "switch", className: "pixel-toggle__native" }),
            React.createElement("span", { className: "pixel-toggle__track", "aria-hidden": "true" },
                React.createElement("span", null))));
    }
    PixelUI.Toggle = Toggle;
    function Slider({ label, value, min, max, unit = '', id: suppliedId, className, ...props }) {
        const generatedId = React.useId();
        const id = suppliedId ?? generatedId;
        const percentage = max === min ? 0 : ((value - min) / (max - min)) * 100;
        return (React.createElement("label", { className: classNames('pixel-slider', className), htmlFor: id },
            React.createElement("span", { className: "pixel-field__label" },
                label,
                React.createElement("output", { htmlFor: id },
                    value,
                    unit)),
            React.createElement("input", { ...props, id: id, type: "range", value: value, min: min, max: max, style: { '--component-slider-progress': `${percentage}%` } })));
    }
    PixelUI.Slider = Slider;
    function Badge({ children, tone = 'neutral' }) {
        return React.createElement("span", { className: classNames('pixel-badge', `pixel-tone--${tone}`) }, children);
    }
    PixelUI.Badge = Badge;
    function Tooltip({ content, children, placement = 'top' }) {
        const id = React.useId();
        return (React.createElement("span", { className: "pixel-tooltip-anchor" },
            React.cloneElement(children, { 'aria-describedby': id }),
            React.createElement("span", { id: id, role: "tooltip", className: `pixel-tooltip pixel-tooltip--${placement}` }, content)));
    }
    PixelUI.Tooltip = Tooltip;
    function Card({ title, eyebrow, actions, children, className, ...props }) {
        return (React.createElement("section", { ...props, className: classNames('pixel-card', className) },
            (title || eyebrow || actions) && (React.createElement("header", { className: "pixel-card__header" },
                React.createElement("div", null,
                    eyebrow && React.createElement("span", { className: "pixel-kicker" }, eyebrow),
                    title && React.createElement("h3", null, title)),
                actions)),
            React.createElement("div", { className: "pixel-card__body" }, children)));
    }
    PixelUI.Card = Card;
    function LoadingState({ label = 'Loading' }) {
        return (React.createElement("div", { className: "pixel-state", role: "status", "aria-live": "polite" },
            React.createElement("span", { className: "pixel-spinner", "aria-hidden": "true" }),
            React.createElement("strong", null, label)));
    }
    PixelUI.LoadingState = LoadingState;
    function EmptyState({ title, description, action }) {
        return (React.createElement("div", { className: "pixel-state" },
            React.createElement("span", { className: "pixel-state__glyph", "aria-hidden": "true" }, "\u25C7"),
            React.createElement("strong", null, title),
            description && React.createElement("p", null, description),
            action));
    }
    PixelUI.EmptyState = EmptyState;
    function ErrorState({ title, description, action }) {
        return (React.createElement("div", { className: "pixel-state pixel-state--error", role: "alert" },
            React.createElement("span", { className: "pixel-state__glyph", "aria-hidden": "true" }, "!"),
            React.createElement("strong", null, title),
            description && React.createElement("p", null, description),
            action));
    }
    PixelUI.ErrorState = ErrorState;
    function classNames(...values) {
        return values.filter((value) => typeof value === 'string' && value.length > 0).join(' ');
    }
    PixelUI.classNames = classNames;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    PixelUI.PixelButton = PixelUI.Button;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    function Stack({ gap = 4, align = 'stretch', className, children, ...props }) {
        return (React.createElement("div", { ...props, className: PixelUI.classNames('pixel-stack', className), "data-align": align, style: { '--layout-gap': `var(--pixel-space-${gap})` } }, children));
    }
    PixelUI.Stack = Stack;
    function Row({ gap = 3, align = 'center', wrap = false, justify = 'start', className, children, ...props }) {
        return (React.createElement("div", { ...props, className: PixelUI.classNames('pixel-row', className), "data-align": align, "data-justify": justify, "data-wrap": wrap, style: { '--layout-gap': `var(--pixel-space-${gap})` } }, children));
    }
    PixelUI.Row = Row;
    function Grid({ columns = 'auto', gap = 4, minColumnWidth = '240px', className, children, ...props }) {
        const template = columns === 'auto'
            ? `repeat(auto-fit, minmax(min(100%, ${minColumnWidth}), 1fr))`
            : `repeat(${Math.max(1, columns)}, minmax(0, 1fr))`;
        return (React.createElement("div", { ...props, className: PixelUI.classNames('pixel-grid', className), style: {
                '--layout-gap': `var(--pixel-space-${gap})`,
                '--grid-template': template,
            } }, children));
    }
    PixelUI.Grid = Grid;
    function Divider({ orientation = 'horizontal', label }) {
        return (React.createElement("div", { className: `pixel-divider pixel-divider--${orientation}`, role: "separator", "aria-orientation": orientation }, label && React.createElement("span", null, label)));
    }
    PixelUI.Divider = Divider;
    function ScrollArea({ label, maxHeight = '100%', className, children, ...props }) {
        return (React.createElement("div", { ...props, className: PixelUI.classNames('pixel-scroll-area', className), role: "region", "aria-label": label, tabIndex: 0, style: { maxHeight } }, children));
    }
    PixelUI.ScrollArea = ScrollArea;
    function Header({ brand, actions, children, className, ...props }) {
        return (React.createElement("header", { ...props, className: PixelUI.classNames('pixel-header', className) },
            React.createElement("div", { className: "pixel-header__brand" }, brand),
            React.createElement("div", { className: "pixel-header__content" }, children),
            React.createElement("div", { className: "pixel-header__actions" }, actions)));
    }
    PixelUI.Header = Header;
    function Sidebar({ label = 'Application navigation', items = [], activeId, onNavigate, children, className, ...props }) {
        return (React.createElement("aside", { ...props, className: PixelUI.classNames('pixel-sidebar', className) },
            React.createElement("nav", { "aria-label": label },
                items.map((item) => (React.createElement("button", { key: item.id, type: "button", className: "pixel-sidebar__item", "aria-current": item.id === activeId ? 'page' : undefined, onClick: () => onNavigate?.(item.id) },
                    item.icon && React.createElement("span", { "aria-hidden": "true" }, item.icon),
                    React.createElement("span", null, item.label)))),
                children)));
    }
    PixelUI.Sidebar = Sidebar;
    function ContentPanel({ title, description, children, className, ...props }) {
        return (React.createElement("section", { ...props, className: PixelUI.classNames('pixel-content-panel', className) },
            (title || description) && (React.createElement("header", null,
                title && React.createElement("h2", null, title),
                description && React.createElement("p", null, description))),
            children));
    }
    PixelUI.ContentPanel = ContentPanel;
    function ApplicationShell({ header, sidebar, children, visible }) {
        return (React.createElement("main", { className: "pixel-shell", "data-visible": visible, "aria-hidden": !visible, inert: !visible ? '' : undefined },
            React.createElement("div", { className: "pixel-application" },
                header,
                sidebar,
                React.createElement("div", { className: "pixel-application__content" }, children))));
    }
    PixelUI.ApplicationShell = ApplicationShell;
    ...
    PixelUI.OverlayLayer = OverlayLayer;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    const OverlayContext = React.createContext(null);
    function OverlayProvider({ children, onDepthChange }) {
        ...
        return React.createElement(OverlayContext.Provider, { value: value }, children);
    }
    PixelUI.OverlayProvider = OverlayProvider;
    function useOverlayRegistration(open) {
    ...
    function OverlayFrame({ open, title, description, onClose, dismissible = true, className, children, footer, role = 'dialog', }) {
        ...
        React.useEffect(() => {
            ...
            if (!open || !panel)
                return undefined;
            return PixelUI.activateFocusTrap(panel, () => {
                if (dismissible)
                    closeRef.current();
            }, registration.isTopMost).deactivate;
        }, [dismissible, open, registration.isTopMost]);
        if (!open)
            return null;
        return (React.createElement(PixelUI.OverlayLayer, null,
            React.createElement("div", { className: "pixel-overlay", "data-overlay-id": registration.id },
                React.createElement("div", { ref: panelRef, className: PixelUI.classNames('pixel-overlay__panel', className), role: role, "aria-modal": "true", "aria-labelledby": titleId, "aria-describedby": description ? descriptionId : undefined, tabIndex: -1 },
                    React.createElement("header", { className: "pixel-overlay__header" },
                        React.createElement("div", null,
                            React.createElement("span", { className: "pixel-kicker" }, "Pixel OS"),
                            React.createElement("h2", { id: titleId }, title),
                            description && React.createElement("p", { id: descriptionId }, description)),
                        dismissible && React.createElement(PixelUI.IconButton, { label: `Close ${title}`, icon: "\u00D7", tone: "neutral", onClick: onClose })),
                    React.createElement("div", { className: "pixel-overlay__body" }, children),
                    footer && React.createElement("footer", { className: "pixel-overlay__footer" }, footer)))));
    }
    function Modal(props) {
        return React.createElement(OverlayFrame, { ...props, className: PixelUI.classNames('pixel-modal', props.className) });
    }
    PixelUI.Modal = Modal;
    function ConfirmDialog({ open, title, description, confirmLabel = 'Confirm', cancelLabel = 'Cancel', tone = 'danger', onConfirm, onCancel, }) {
        return (React.createElement(OverlayFrame, { open: open, title: title, description: description, onClose: onCancel, role: "alertdialog", footer: (React.createElement(PixelUI.Row, { justify: "end" },
                React.createElement(PixelUI.Button, { tone: "neutral", onClick: onCancel }, cancelLabel),
                React.createElement(PixelUI.Button, { tone: tone, onClick: onConfirm }, confirmLabel))) },
            React.createElement("p", { className: "pixel-dialog__message" }, "This action requires an explicit decision.")));
    }
    PixelUI.ConfirmDialog = ConfirmDialog;
    PixelUI.Dialog = ConfirmDialog;
    function Drawer({ side = 'right', ...props }) {
        return React.createElement(OverlayFrame, { ...props, className: `pixel-drawer pixel-drawer--${side}` });
    }
    PixelUI.Drawer = Drawer;
    ...
    function ToastProvider({ children }) {
        ...
        const notify = React.useCallback((input) => {
            sequence.current += 1;
        ...
        const api = React.useMemo(() => ({ notify, dismiss, clear }), [clear, dismiss, notify]);
        return (React.createElement(ToastContext.Provider, { value: api },
            children,
            React.createElement(PixelUI.OverlayLayer, null,
                React.createElement("div", { className: "pixel-toast-region", "aria-label": "Notifications", "aria-live": "polite" }, toasts.map((toast) => (React.createElement("article", { key: toast.id, className: `pixel-toast pixel-toast--${toast.tone ?? 'info'}`, role: "status" },
                    React.createElement("span", { className: "pixel-toast__energy", "aria-hidden": "true" }),
                    React.createElement("div", null,
                        React.createElement("strong", null, toast.title),
                        toast.message && React.createElement("p", null, toast.message)),
                    React.createElement(PixelUI.IconButton, { label: `Dismiss ${toast.title}`, icon: "\u00D7", tone: "neutral", size: "small", onClick: () => dismiss(toast.id) }))))))));
    }
    PixelUI.ToastProvider = ToastProvider;
    function useToast() {
    ...
    PixelUI.useToast = useToast;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    function Tabs({ label, tabs, selectedId, onChange }) {
        ...
        const onKeyDown = (event) => {
            ...
            const next = PixelUI.nextKeyboardIndex(current, enabled.length, event.key, 'horizontal');
            const nextTab = enabled[next];
            if (!nextTab)
                return;
            event.preventDefault();
            select(nextTab.id);
            const button = event.currentTarget.querySelector(`[data-tab-id="${nextTab.id}"]`);
            button?.focus();
        };
        const active = tabs.find((tab) => tab.id === activeId) ?? tabs[0];
        return (React.createElement("div", { className: "pixel-tabs" },
            React.createElement("div", { className: "pixel-tabs__list", role: "tablist", "aria-label": label, onKeyDown: onKeyDown }, tabs.map((tab) => (React.createElement("button", { key: tab.id, type: "button", role: "tab", id: `${instanceId}-tab-${tab.id}`, "data-tab-id": tab.id, "aria-selected": tab.id === active?.id, "aria-controls": `${instanceId}-panel-${tab.id}`, tabIndex: tab.id === active?.id ? 0 : -1, disabled: tab.disabled, onClick: () => select(tab.id) }, tab.label)))),
            active && (React.createElement("div", { className: "pixel-tabs__panel", role: "tabpanel", id: `${instanceId}-panel-${active.id}`, "aria-labelledby": `${instanceId}-tab-${active.id}`, tabIndex: 0 }, active.content))));
    }
    PixelUI.Tabs = Tabs;
    function DataTable({ label, columns, rows, rowKey, emptyTitle = 'No records available', }) {
        if (rows.length === 0)
            return React.createElement(PixelUI.EmptyState, { title: emptyTitle });
        return (React.createElement("div", { className: "pixel-table-wrap", role: "region", "aria-label": label, tabIndex: 0 },
            React.createElement("table", { className: "pixel-table" },
                React.createElement("caption", { className: "pixel-visually-hidden" }, label),
                React.createElement("thead", null,
                    React.createElement("tr", null, columns.map((column) => (React.createElement("th", { key: column.id, scope: "col", "data-align": column.align ?? 'left' }, column.header))))),
                React.createElement("tbody", null, rows.map((row) => (React.createElement("tr", { key: rowKey(row) }, columns.map((column) => (React.createElement("td", { key: column.id, "data-align": column.align ?? 'left' }, column.render(row)))))))))));
    }
    PixelUI.DataTable = DataTable;
    function ContextMenu({ label, trigger, items }) {
        ...
        const onKeyDown = (event) => {
            ...
            if (!['ArrowUp', 'ArrowDown', 'Home', 'End'].includes(event.key))
                return;
            const buttons = Array.from(event.currentTarget.querySelectorAll('[role="menuitem"]:not([disabled])'));
            const current = Math.max(0, buttons.indexOf(document.activeElement));
            const next = PixelUI.nextKeyboardIndex(current, buttons.length, event.key, 'vertical');
            event.preventDefault();
            buttons[next]?.focus();
        };
        return (React.createElement("div", { className: "pixel-context", ref: menuRef },
            React.createElement("button", { ref: triggerRef, type: "button", className: "pixel-context__trigger", "aria-haspopup": "menu", "aria-expanded": open, "aria-controls": menuId, onClick: () => {
                    setOpen((current) => !current);
                    window.setTimeout(() => focusItem(0), 0);
                } }, trigger),
            open && (React.createElement("div", { id: menuId, className: "pixel-context__menu", role: "menu", "aria-label": label, onKeyDown: onKeyDown }, items.map((item) => (React.createElement("button", { key: item.id, type: "button", role: "menuitem", disabled: item.disabled, "data-destructive": item.destructive || undefined, onClick: () => {
                    item.onSelect();
                    setOpen(false);
                } }, item.label)))))));
    }
    ...
    function Showcase({ bridge }) {
        ...
        const tableColumns = [
            { id: 'module', header: 'Module', render: (row) => row.module },
            {
                id: 'status',
                header: 'Status',
                render: (row) => (React.createElement(PixelUI.Badge, { tone: row.status === 'Ready' ? 'success' : row.status === 'Review' ? 'warning' : 'neutral' }, row.status)),
            },
        ...
        const runBridgeExample = async () => {
            ...
            try {
                const result = await bridge.request('diagnostics.example', { message: 'Showcase bridge request' }, { timeoutMs: 1500 });
        ...
        return (React.createElement(PixelUI.ScrollArea, { label: "Pixel UI component showcase", maxHeight: "calc(100vh - var(--pixel-size-header))" },
            React.createElement(PixelUI.Stack, { gap: 6, className: "pixel-showcase" },
                React.createElement("section", { className: "pixel-showcase__hero" },
                    React.createElement("span", { className: "pixel-kicker" }, "Foundation v0.2.1 \u00B7 Developer only"),
                    React.createElement("h1", null, "Pixel UI Kit"),
                    React.createElement("p", null, "Reusable controls, accessible overlays, layouts, notifications, and the typed bridge in the approved Pixel OS material system."),
                    React.createElement(PixelUI.Row, { wrap: true },
                        React.createElement(PixelUI.Badge, { tone: "success" }, "21 components"),
                        React.createElement(PixelUI.Badge, { tone: "primary" }, "Strict TypeScript"),
                        React.createElement(PixelUI.Badge, { tone: "neutral" }, "Offline NUI"))),
                React.createElement(PixelUI.Grid, { columns: "auto", minColumnWidth: "320px", gap: 5 },
                    React.createElement(PixelUI.Card, { title: "Actions", eyebrow: "Buttons", actions: React.createElement(PixelUI.Badge, { tone: "primary" }, "Interactive") },
                        React.createElement(PixelUI.Stack, null,
                            React.createElement(PixelUI.Row, { wrap: true },
                                React.createElement(PixelUI.Button, null, "Primary"),
                                React.createElement(PixelUI.Button, { tone: "neutral" }, "Neutral"),
                                React.createElement(PixelUI.Button, { tone: "success" }, "Success"),
                                React.createElement(PixelUI.Button, { tone: "warning" }, "Warning"),
                                React.createElement(PixelUI.Button, { tone: "danger" }, "Danger"),
                                React.createElement(PixelUI.Button, { disabled: true }, "Disabled"),
                                React.createElement(PixelUI.Button, { loading: true }, "Loading")),
                            React.createElement(PixelUI.Row, null,
                                React.createElement(PixelUI.Tooltip, { content: "Icon buttons require an accessible label" },
                                    React.createElement(PixelUI.IconButton, { label: "Energy settings", icon: "\u2699" })),
                                React.createElement(PixelUI.IconButton, { label: "Close preview", icon: "\u00D7", tone: "neutral" })))),
                    React.createElement(PixelUI.Card, { title: "Form controls", eyebrow: "Inputs" },
                        React.createElement(PixelUI.Stack, null,
                            React.createElement(PixelUI.Input, { label: "Operator name", value: input, onChange: (event) => setInput(event.currentTarget.value), hint: "A visible native label is always provided." }),
                            React.createElement(PixelUI.Textarea, { label: "Design note", rows: 3, value: notes, onChange: (event) => setNotes(event.currentTarget.value) }),
                            React.createElement(PixelUI.Select, { label: "Target module", value: select, onChange: (event) => setSelect(event.currentTarget.value), options: [
                                    { value: 'inventory', label: 'Pixel Inventory' },
                                    { value: 'character', label: 'Pixel Character' },
                                    { value: 'admin', label: 'Pixel Admin' },
                                ] }),
                            React.createElement(PixelUI.Checkbox, { label: "Enable telemetry preview", description: "Development data only", checked: checked, onChange: (event) => setChecked(event.currentTarget.checked) }),
                            React.createElement(PixelUI.Toggle, { label: "Energy accent", description: "Uses switch semantics", checked: toggled, onChange: (event) => setToggled(event.currentTarget.checked) }),
                            React.createElement(PixelUI.Slider, { label: "Interface scale", value: slider, min: 50, max: 100, unit: "%", onChange: (event) => setSlider(Number(event.currentTarget.value)) }))),
                    React.createElement(PixelUI.Card, { title: "Overlays", eyebrow: "Focus managed" },
                        React.createElement(PixelUI.Stack, null,
                            React.createElement(PixelUI.Row, { wrap: true },
                                React.createElement(PixelUI.Button, { onClick: () => setModal(true) }, "Open modal"),
                                React.createElement(PixelUI.Button, { tone: "danger", onClick: () => setDialog(true) }, "Confirm dialog"),
                                React.createElement(PixelUI.Button, { tone: "neutral", onClick: () => setDrawer(true) }, "Open drawer")),
                            React.createElement(PixelUI.Divider, { label: "Notifications" }),
                            React.createElement(PixelUI.Row, { wrap: true },
                                React.createElement(PixelUI.Button, { tone: "neutral", onClick: () => toast.notify({ title: 'Information', message: 'The NUI bridge is ready.', tone: 'info' }) }, "Info toast"),
                                React.createElement(PixelUI.Button, { tone: "success", onClick: () => toast.notify({ title: 'Saved', message: 'Local showcase state updated.', tone: 'success' }) }, "Success"),
                                React.createElement(PixelUI.Button, { tone: "warning", onClick: () => toast.notify({ title: 'Review', message: 'Live Enhanced QA remains pending.', tone: 'warning' }) }, "Warning"),
                                React.createElement(PixelUI.Button, { tone: "danger", onClick: () => toast.notify({ title: 'Rejected', message: 'Invalid authority data was blocked.', tone: 'error' }) }, "Error")))),
                    React.createElement(PixelUI.Card, { title: "System states", eyebrow: "Feedback" },
                        React.createElement(PixelUI.Grid, { columns: 3, minColumnWidth: "140px", gap: 3 },
                            React.createElement(PixelUI.LoadingState, { label: "Synchronizing" }),
                            React.createElement(PixelUI.EmptyState, { title: "No records", description: "Create the first entry." }),
                            React.createElement(PixelUI.ErrorState, { title: "Request failed", description: "Try again safely." })))),
                React.createElement(PixelUI.Card, { title: "Navigation patterns", eyebrow: "Keyboard ready" },
                    React.createElement(PixelUI.Tabs, { label: "Component categories", tabs: [
                            { id: 'overview', label: 'Overview', content: React.createElement("p", null, "Use Left/Right, Home, and End to move between tabs.") },
                            { id: 'tokens', label: 'Tokens', content: React.createElement("p", null, "All visual values come from the generated Pixel token source.") },
                            { id: 'disabled', label: 'Unavailable', content: null, disabled: true },
                        ] })),
                React.createElement(PixelUI.Card, { title: "Data table and context menu", eyebrow: "Structured data", actions: (React.createElement(PixelUI.ContextMenu, { label: "Table actions", trigger: React.createElement("span", null, "Actions \u25BE"), items: [
                    ...
                    React.createElement(PixelUI.DataTable, { label: "Pixel module readiness", columns: tableColumns, rows: rows, rowKey: (row) => row.id })),
                React.createElement(PixelUI.Grid, { columns: "auto", minColumnWidth: "260px" },
                    React.createElement(PixelUI.ContentPanel, { title: "Content panel", description: "A focused application work surface." },
                        React.createElement(PixelUI.Stack, { gap: 2 },
                            React.createElement("span", null, "Stack controls vertical rhythm."),
                            React.createElement(PixelUI.Row, { justify: "between" },
                                React.createElement("span", null, "Row aligns content."),
                                React.createElement(PixelUI.Badge, null, "Layout")))),
                    React.createElement(PixelUI.ContentPanel, { title: "Typed bridge", description: "Browser mode uses a safe local callback mock." },
                        React.createElement(PixelUI.Stack, { gap: 3 },
                            React.createElement("code", null, bridgeResult),
                            React.createElement(PixelUI.Button, { onClick: () => void runBridgeExample() }, "Run correlated request"))))),
            React.createElement(PixelUI.Modal, { open: modal, title: "Accessible modal", description: "Tab is trapped here, Escape closes, and focus returns to the trigger.", onClose: () => setModal(false), footer: React.createElement(PixelUI.Button, { onClick: () => setModal(false) }, "Done") },
                React.createElement(PixelUI.Input, { label: "Focusable field", placeholder: "Try keyboard navigation" }),
                React.createElement(PixelUI.Button, { tone: "neutral", onClick: () => setDialog(true) }, "Open nested dialog")),
            React.createElement(PixelUI.ConfirmDialog, { open: dialog, title: "Confirm local action", description: "This showcase action never reaches production authority.", onCancel: () => setDialog(false), onConfirm: () => {
                    setDialog(false);
                    toast.notify({ title: 'Confirmed', tone: 'success' });
                } }),
            React.createElement(PixelUI.Drawer, { open: drawer, title: "Navigation drawer", description: "Drawers share the same focus and Escape contract.", onClose: () => setDrawer(false), footer: React.createElement(PixelUI.Button, { fullWidth: true, onClick: () => setDrawer(false) }, "Close drawer") },
                React.createElement(PixelUI.Stack, null,
                    React.createElement(PixelUI.Button, { tone: "neutral" }, "Profile"),
                    React.createElement(PixelUI.Button, { tone: "neutral" }, "Permissions"),
                    React.createElement(PixelUI.Button, { tone: "neutral" }, "Audit trail")))));
    }
    PixelUI.Showcase = Showcase;
})(PixelUI || (PixelUI = {}));
var PixelUI;
(function (PixelUI) {
    function bridgeDiagnostic(level, message, fields) {
        if (typeof window.GetParentResourceName === 'function')
            return;
    ...
    function App() {
        ...
        if (!busRef.current)
            busRef.current = new PixelUI.NuiEventBus(bridgeDiagnostic);
        if (!bridgeRef.current) {
            bridgeRef.current = new PixelUI.NuiClient(undefined, bridgeDiagnostic);
            PixelUI.installBrowserMocks(bridgeRef.current);
        }
        const bus = busRef.current;
        const bridge = bridgeRef.current;
        const browserMode = typeof window.GetParentResourceName !== 'function';
        const [state, setState] = React.useState(browserMode ? PixelUI.createBrowserMockState() : {
            visible: false,
            activeOwner: null,
            focus: false,
            cursor: false,
        ...
        React.useEffect(() => {
            ...
            void bridge.request('ui.ready', {}, { timeoutMs: 5000 })
                ...
                .catch((error) => bridgeDiagnostic('warn', 'Ready handshake failed', { error: error instanceof Error ? error.message : 'unknown' }));
            return () => {
            ...
            const onKeyDown = (event) => {
                ...
                void bridge.request('ui.release', { owner: state.activeOwner })
                    ...
                    if (!result.state.visible)
                        bridge.cancelPending('Pixel UI closed');
                })
                    .catch(() => undefined);
            };
        ...
        const reportOverlayDepth = React.useCallback((depth) => {
        ...
        return (React.createElement(PixelUI.OverlayProvider, { onDepthChange: reportOverlayDepth },
            React.createElement(PixelUI.ToastProvider, null,
                React.createElement(PixelUI.ApplicationShell, { visible: state.visible, header: (React.createElement(PixelUI.Header, { brand: (React.createElement("span", { className: "pixel-wordmark" },
                            React.createElement("span", { "aria-hidden": "true" }, "PN"),
                            React.createElement("strong", null,
                                "PIXEL ",
                                React.createElement("small", null, "NETWORK")))), actions: (React.createElement(PixelUI.Row, null,
                            browserMode && React.createElement(PixelUI.Badge, { tone: "warning" }, "Browser mock"),
                            React.createElement(PixelUI.Badge, { tone: state.focus ? 'success' : 'neutral' }, state.focus ? 'Focus owned' : 'Passive'),
                            React.createElement(PixelUI.IconButton, { label: "Close active Pixel interface", icon: "\u00D7", tone: "neutral", onClick: closeActive }))) },
                        React.createElement("span", { className: "pixel-header__title" }, showcaseVisible ? 'Developer showcase' : 'Shared application shell'))), sidebar: (React.createElement(PixelUI.Sidebar, { items: [
                            { id: 'showcase', label: 'Components', icon: '◇' },
                            { id: 'bridge', label: 'Bridge', icon: '↔' },
                            { id: 'accessibility', label: 'Accessibility', icon: '◎' },
                        ], activeId: "showcase" })) }, showcaseVisible
                    ? React.createElement(PixelUI.Showcase, { bridge: bridge })
                    : (React.createElement(PixelUI.ContentPanel, { title: "Pixel UI is ready", description: "This shared surface is waiting for the active Pixel module to provide its application view." },
                        React.createElement(PixelUI.EmptyState, { title: "No development panel selected", description: "The component showcase remains disabled unless explicitly enabled.", action: React.createElement(PixelUI.Button, { onClick: closeActive }, "Close interface") })))))));
    }
    PixelUI.App = App;
})(PixelUI || (PixelUI = {}));
const pixelRootElement = document.getElementById('root');
if (!pixelRootElement)
    throw new Error('Pixel UI root element is missing');
ReactDOM.createRoot(pixelRootElement).render(React.createElement(React.StrictMode, null,
    React.createElement(PixelUI.App, null)));
